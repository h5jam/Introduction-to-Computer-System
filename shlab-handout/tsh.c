...
void eval(char *cmdline) 
{
	...
	/* input seperated by parseline store to argv*/ 
	...
	if(!builtin_cmd(argv)){
		...
		//signal unblock,cuz add job
		sigprocmask(SIG_UNBLOCK, &mask, NULL);

		if(!bg){
			//wait until foreground job ends
			waitfg(pid, STDOUT_FILENO);
		}
		else{
			//background job info print
...
void waitfg(pid_t pid, int output_fd)
{	
	// get current job struct
	struct job_t *job_temp = getjobpid(jobs, pid);
	
	//return if it were not for job struct
	if(!job_temp)
		return;
	
	//wait until foreground job ends
	while(pid == fgpid(job_temp) && job_temp->state == FG)
		sleep(1);
...
void sigchld_handler(int sig) 
{
	/* SIGCHLD occur when child process exit */
	int child_status = 0;
	pid_t pid;
	
	/* wait for return of child process */
